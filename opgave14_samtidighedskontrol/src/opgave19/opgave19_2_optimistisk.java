...
public class opgave19_2_optimistisk {
...
//            stmt.execute("set transaction isolation level snapshot");
            ...
            if(proceed){
                ...
                if(resAccount.next()){
                    balance = resAccount.getInt(2);
                    System.out.println("AccountNumber: " + resAccount.getInt(1) + " Balance: " +
                                balance + " Owner: " + resAccount.getString(3));
                ...
                if(proceed){
                    ResultSet updatedRes = stmt.executeQuery("select saldo from konto where kontonr = " + accountNumber);
                    if(updatedRes.next()) {
                        if (updatedRes.getInt(1) == balance) {
                            stmt.execute("update konto set saldo = " + amount + " where kontonr = " + accountNumber);
                            minConnection.commit();
                        } else {
                            proceed = false;
                            System.out.println("Failed because of changed data");
                        }
                    }
                }
